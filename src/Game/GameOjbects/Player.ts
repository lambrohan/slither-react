...
export class Player {
  snakeHead:Phaser.GameObjects.Sprite|null=null;
  ...
  movePlayer (x: number,y: number) {
    ...
    this.snakeHead.body.velocity.copy(this.scene.physics.velocityFromAngle(this.snakeHead.angle, 300));
  ...
  grow(food:Food){
    ...
    for (let i = 0; i <= food.size; i++) {
      this.snakeSection[this.numSnakeSections] = this.scene?.add.sprite(this.snakeSection[this.numSnakeSections - 1].x + this.snakeSpacer, this.snakeSection[this.numSnakeSections - 1].y + this.snakeSpacer, 'body');
      ...
      for (let i = this.snakePath.length; i <= this.numSnakeSections * this.snakeSpacer; i++) {
        this.snakePath[i] = new Phaser.Geom.Point(this.snakePath[i - 1].x, this.snakePath[i - 1].y);
      }
    }
  }
